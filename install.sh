...
# --- Configuration ---
GITHUB_REPO="USER/debian_base_scripts"  # TODO: Update with actual GitHub username/org
GITHUB_BRANCH="main"
...
if [ ! -f "/home/admin/.bashrc" ]; then
  ...
  if [[ ! "$RUN_INIT" =~ ^[Nn]$ ]]; then
    ...
    echo "  ${GREEN}${INSTALL_DIR}/init-service-container.sh${NC}"
  fi
else
  echo -e "${GREEN}System appears to be already initialized.${NC}"
  echo ""
  log_info "Recommended next steps:"
  echo "  1. Run: ${GREEN}${INSTALL_DIR}/setup-enhanced-motd.sh${NC}"
  echo "  2. Run: ${GREEN}${INSTALL_DIR}/install-podman-cockpit.sh${NC}"
  echo "  3. Deploy services with: ${GREEN}${INSTALL_DIR}/create-podman-service.sh${NC}"
fi
